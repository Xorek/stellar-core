...
GetAndUnzipRemoteFileWork::onReset()
{
    ...
    CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": downloading";
...
GetAndUnzipRemoteFileWork::onSuccess()
{
    if (mGunzipFileWork)
    {
        ...
        {
            CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": .xdr not found";
            return WORK_FAILURE_RETRY;
        }
        else
        {
    ...
    {
        CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": renaming .gz.tmp to .gz";
        if (fs::exists(mFt.localPath_gz()) && std::remove(mFt.localPath_gz().c_str()))
        {
            CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": failed to remove .gz";
            return WORK_FAILURE_RETRY;
        }

        if (std::rename(mFt.localPath_gz_tmp().c_str(), mFt.localPath_gz().c_str()))
        {
            CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": failed to rename .gz.tmp to .gz";
            return WORK_FAILURE_RETRY;
        }

        CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": renamed .gz.tmp to .gz";
    }

    if (!fs::exists(mFt.localPath_gz()))
    {
        CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": .gz not found";
        return WORK_FAILURE_RETRY;
    }

    CLOG(INFO, "History") << "Downloading and unzipping " << mFt.remoteName() << ": unzipping";
